...
if (isset($_POST['create'])) {
    ...
    if (isset($_POST['hide'])) {
        hideCompetition($_POST['comp_name']);
    } else {
        deleteCompetition($_POST['comp_name']);
    }
...
$check_status_hidden = (getAssociatedCompInfo($comp, 'hidden') ? 'checked' : '');
...
<form method="get" class="filled border" style="text-align: center; margin: 6px;">
    <fieldset>
        ...
        <select name="comp_name" onchange="this.form.submit()" style="margin-bottom: 6px;">
            ...
            $comp_names_stmt = $sql_conn->prepare("SELECT competition_name FROM competitions WHERE NOT hidden");
...
<form method="post" class="filled border" style="text-align: center;">
    <fieldset>
        ...
        <input id="hide" name="hide" type="submit" value="Hide"
               style="color: orange;" <?php if (is_null($comp)) echo 'disabled'; ?>>
